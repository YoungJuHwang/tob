...
<table id="tab_admin_member">
	<tr>
		<th>아이디</th>
		<td id="member_userid">${member.userid}</td>
	</tr>
	<tr>
		<th>비밀번호</th>
		...
		$("#delete_member").click(function() {
			if (confirm("정말 삭제하시겠습니까?")) {
				$.ajax("${context}/admin/Admin.do?page=delete",{
					data : {
						id : $("#member_userid").text(),
					},
					dataType : "json",
					success : function(data) {
					/* 	$("#div_admin_member").html(data.result);
						//부모창의 회원목록 버튼을 클릭하게함
						opener.document.getElementById("mgmt_member").click(); */
						opener.document.getElementById("mgmt_member").click(); 
					},
					error : function() {
			...
			if(confirm("정말 수정하시겠습니까?")){
				...
				$("#update_member").click(function() {
					if(confirm("정말 등록하시겠습니까?")){
						console.log(userid + password + phone + email + addr);
